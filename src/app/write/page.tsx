...
const TEMP_STORAGE_KEY = 'writePageTempData'; // 임시저장 키
...
const WritePage = () => {
  ...
  useEffect(() => {
    ...
    if (tempData) {
      ...
      setBodySize(parsedData.bodySize || { height: '', weight: '' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert('저장 성공!');
      clearTempData(); // 게시글 저장 후 임시저장 데이터 삭제
      router.push('/posts');
    } catch (error) {
      ...
      <div className="w-full max-w-[1200px] mx-auto border border-gray-300 rounded-md shadow-lg p-8">
        <div className="flex justify-between items-center mb-8">
          <button
            ...
            className="px-6 py-2 bg-gray-200 border border-gray-300 rounded-lg"
          >
            임시저장
          </button>
          <button
